...
public class RSAUtil {
    ...
    public static final String PUBLIC_KEY_STR = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCeVDqArpsUZC87Jgk+j9l+uX1MzgDS9vOWQNHlpI7/j+F92SkupnvLAlKoY2sg/WeS/PfrFFW5CAKda0nIsIDtzUpVvoSV5BYlwnfDnWA/FsoeW0NeatbuahWBBhX6eAILI1fX2uLT3aPAc9RbpPq6Md2StGBFFrUKmfT8puiljwIDAQAB";
    ...
    static {
        PUBLIC_KEY = keyStrToPublicKey(PUBLIC_KEY_STR);
    }
    ...
    public static String encrypt(String content, Key publicKey) throws Exception{
        ...
        byte [] contentData = content.getBytes("UTF8");
        ...
        while (inputLen - offSet > 0) {
            if (inputLen - offSet > MAX_ENCRYPT_BLOCK) {
                cache = cipher.doFinal(contentData, offSet, MAX_ENCRYPT_BLOCK);
            } else {
        ...
        byte[] encodeContent = out.toByteArray();
        out.close();
        return Base64.encodeToString(encodeContent, sBase64Mode);
    }

    public static String decrypt(String content, Key publicKey) throws Exception{
        byte[] byteArray = Base64.decode(content, sBase64Mode);
        Cipher cipher = Cipher.getInstance(sTransform);
        ...
        while (inputLen - offSet > 0) {
            if (inputLen - offSet > MAX_DECRYPT_BLOCK) {
                cache = cipher.doFinal(byteArray, offSet, MAX_DECRYPT_BLOCK);
            } else {
    ...
    /*
       使用公钥加密数据，结果用Base64转码
    */
    public static String encryptDataByPublicKey(String srcData, PublicKey publicKey){
        byte[] bytes = new byte[0];
        try {
            bytes = Base64.decode(srcData.getBytes("UTF-8"), sBase64Mode);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        byte[] resultBytes = processData(bytes,publicKey,Cipher.ENCRYPT_MODE);
