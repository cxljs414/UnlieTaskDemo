 ...
 public class DividerItemDecoration extends RecyclerView.ItemDecoration {
     ...
     * */
     ...
     public DividerItemDecoration(Context context, int orientation) {
         this(context,orientation,context.getResources().getColor(R.color.color_appbar_split_line),0.5f);
     }

    public DividerItemDecoration(Context context, int orientation, float size) {
        this(context,orientation,context.getResources().getColor(R.color.color_appbar_split_line),size);
    }
    ...
    public DividerItemDecoration(Context context, int orientation, int color, float size) {
     ...
     private void drawVertical(Canvas canvas, RecyclerView parent){
         ...
         for (int i = 0; i < childSize; i++) {
             ...
             final float top = new BigDecimal(child.getBottom() + layoutParams.bottomMargin).floatValue();
             final float bottom = new BigDecimal((double) top + (double) mItemSize).floatValue();
     ...
     private void drawHorizontal(Canvas canvas, RecyclerView parent){
         ...
         for (int i = 0; i < childSize; i++) {
             final View child = parent.getChildAt(i);
             RecyclerView.LayoutParams layoutParams = (RecyclerView.LayoutParams) child.getLayoutParams();
             final float left = new BigDecimal(child.getRight() + layoutParams.rightMargin).floatValue();
             final float right = new BigDecimal((double) left + (double) Math.max(mItemSize,1.0f)).floatValue();
             canvas.drawRect(left, top, right, bottom, mPaint);
         }
     }
